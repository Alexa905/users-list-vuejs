...
<script>
  ...
  export default {
    ...
    created() {
      this.$http.get(`${this.$router.options.server}/api/user/${this.id}`)
        .then((data) => {
          this.user = data.body;
        })
        .catch((data) => {
    ...
    methods: {
      ...
      deleteUser() {
        this.$http.delete(`${this.$router.options.server}/api/user/${this.id}`)
          .then(() => {
      ...
      updateUser() {
        this.$http.put(`${this.$router.options.server}/api/user/${this.user.id}`, this.user)
          .then(() => {
